...
class OnnxModel:
    ...
    def load(self, model_dir: Path, device: Device, warmup: bool = False, quantized: bool = False):
        ...
        if quantized:
            ...
            assert file_name in [f.name for f in model_dir.iterdir()], f"Quantized model {file_name} not found in {model_dir}"
        else:
            file_name = "model.onnx"
            assert file_name in [f.name for f in model_dir.iterdir()], f"Model {file_name} not found in {model_dir}"
